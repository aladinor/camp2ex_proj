...
def plot_multi_panel(lores):
    for i in range(lores.zhh14.shape[2]):
        ...
        y = lores.range[:] * np.cos(np.deg2rad(lores.azimuth[:]))
        a = ax2.pcolormesh(x[:, :, 0], y[:, :, 0], lores.zhh14[:, :, i],
                           cmap=drpy.graph.cmaps.HomeyerRainbow, vmin=-10, vmax=40)
        ax2.invert_yaxis()
        plt.colorbar(a, ax=ax2, orientation='vertical')

        dbz = lores.zhh14.values.copy()
        dbz[:, 12, i:] = np.nan
        ...
        plt.savefig(f'/home/alfonso/Documents/camp2ex_proj/results/Ka/ref/dual_plot_{i}')
